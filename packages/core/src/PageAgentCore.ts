	...
	#packMacroTool(): Tool<MacroToolInput, MacroToolResult> {
		...
		return {
			...
			execute: async (input: MacroToolInput): Promise<MacroToolResult> => {
				...
				// Record demo step if callback is configured
				if (this.config.onDemoStep && toolName !== 'done' && toolName !== 'ask_user') {
					this.config.onDemoStep(toolName, toolInput as Record<string, unknown>, result)
				}
