...
import { DemoModeManager, type DemoPath, type DemoStep } from './DemoMode'
...
export interface PanelConfig {
	...
	/**
	 * Enable demo mode for recording and replaying agent actions
	 * @default false
	 */
...
export class Panel {
	...
	#isAnimating = false
	#demoMode: DemoModeManager | null = null
	...
	// ========== Demo Mode Public Methods ==========

	/**
	 * Start recording a demo path
	 * @param name - Name for the demo path
	 */
	startDemoRecording(name: string): void {
		if (!this.#demoMode) {
			console.warn('[Panel] Demo mode not enabled. Set enableDemoMode: true in config.')
			return
		}
		this.#demoMode.startRecording(name)
		this.#updateStatusIndicator('thinking')
		this.#statusText.textContent = `Recording demo: ${name}`
		this.show()
	}

	/**
	 * Stop recording and save the demo path
	 */
	stopDemoRecording(): DemoPath | null {
		if (!this.#demoMode) return null
		const path = this.#demoMode.stopRecording()
		if (path) {
			this.#statusText.textContent = `Saved demo: ${path.name} (${path.steps.length} steps)`
		}
		return path
	}
	 ...
	 */
	cancelDemoRecording(): void {
		if (!this.#demoMode) return
		this.#demoMode.cancelRecording()
		this.#statusText.textContent = 'Recording cancelled'
	}

	/**
	 * Replay a saved demo path
	 * @param pathId - ID of the path to replay
	 * @param onStep - Callback for each step during replay
	 */
	async replayDemo(
		pathId: string,
		onStep?: (step: DemoStep, index: number) => Promise<void> | void
	): Promise<void> {
		if (!this.#demoMode) {
			console.warn('[Panel] Demo mode not enabled. Set enableDemoMode: true in config.')
			return
		...
		const path = this.#demoMode.startReplay(pathId)
		if (!path) {
			console.error('[Panel] Demo path not found:', pathId)
			return
		}
		...
		// Replay each step
		while (this.#demoMode.isReplaying) {
			const next = this.#demoMode.getNextStep()
			if (!next) break

			const { step, index } = next
			this.#statusText.textContent = `Demo step ${index + 1}/${path.steps.length}`
		...
		this.#statusText.textContent = 'Demo complete'
		this.#updateStatusIndicator('completed')
	}

	/**
	 * Get all saved demo paths
	 */
	getSavedDemoPaths(): DemoPath[] {
		if (!this.#demoMode) return []
		return this.#demoMode.savedPaths
	}

	/**
	 * Delete a saved demo path
	 */
	deleteDemoPath(pathId: string): boolean {
		if (!this.#demoMode) return false
		return this.#demoMode.deletePath(pathId)
	}

	/**
	 * Record a step during agent execution (called internally)
	 * @internal
	 */
	recordDemoStep(action: string, input: Record<string, unknown>, description?: string): boolean {
		if (!this.#demoMode) return false
		return this.#demoMode.recordStep(action, input, description)
	}
