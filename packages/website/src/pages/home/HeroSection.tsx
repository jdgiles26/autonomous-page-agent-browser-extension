		...
		>
			...
			<div className="max-w-7xl mx-auto text-center relative z-10">
				...
				<div className="relative z-10">
					{/* Mascot row — comic panel energy */}
					<div className="flex items-end justify-center gap-3 sm:gap-6 mb-6 sm:mb-8">
						<img
							src={FRENCHIE_MASCOT}
							alt="Cyber Frenchie"
							className="w-24 sm:w-36 lg:w-44 h-auto drop-shadow-[0_0_24px_rgba(168,85,247,0.55)] select-none pointer-events-none"
							draggable={false}
						/>
						{/* Giant graffiti MAKE AI FREE */}
						<div className="flex flex-col items-center">
							<div className="inline-flex items-center gap-2 px-4 py-2 mb-2 text-xs sm:text-sm font-bold tracking-wide rounded-full border border-purple-400/40 bg-black/60 shadow-[0_0_24px_rgba(168,85,247,0.35)] backdrop-blur-md">
								<span className="w-2 h-2 bg-teal-400 rounded-full animate-pulse shadow-[0_0_10px_#2dd4bf]" aria-hidden="true" />
								<AnimatedGradientText colorFrom="#2dd4bf" colorTo="#c084fc">
									MAKE AI FREE
								</AnimatedGradientText>
							</div>
							<p className="text-[10px] sm:text-xs text-purple-300/80 font-semibold tracking-widest uppercase">
								{isZh ? '街头 × 赛博' : 'Street × Cyber'}
							</p>
						</div>
						<img
							src={HUSKY_MASCOT}
							alt="Cyber Husky"
							className="w-24 sm:w-36 lg:w-44 h-auto drop-shadow-[0_0_24px_rgba(20,184,166,0.55)] select-none pointer-events-none"
							draggable={false}
						/>
					</div>
					...
					<h1
						id="hero-heading"
						className="text-4xl sm:text-5xl lg:text-7xl font-black mb-6 sm:mb-10 mt-2 leading-[1.1] tracking-tight"
						style={{
							textShadow:
									...
									{activeTab === 'try' && (
										<div className="space-y-4">
											<div className="relative">
												...
												>
													{ready ? (isZh ? '执行' : 'Run') : (
														<span className="animate-pulse">{isZh ? '准备中...' : 'Preparing...'}</span>
													)}
												</button>
											</div>
											<p className="text-xs text-gray-500 text-left">
												{isZh ? (
													<>
														...
														<a href="https://github.com/alibaba/page-agent/blob/main/docs/terms-and-privacy.md#2-testing-api-and-demo-disclaimer--terms-of-use" target="_blank" rel="noopener noreferrer" className="underline text-teal-400">使用条款</a>
													</>
												) : (
													<>
														Powered by free testing LLM API. By clicking Run you agree to the{' '}
														<a href="https://github.com/alibaba/page-agent/blob/main/docs/terms-and-privacy.md#2-testing-api-and-demo-disclaimer--terms-of-use" target="_blank" rel="noopener noreferrer" className="underline text-teal-400">Terms of Use</a>
													</>
												)}
											</p>
										</div>
												...
												<ul className="space-y-1.5 text-sm text-gray-300">
													<li className="flex items-start">
														...
														<span>
															...
															{isZh
																? 'Apple 禁止 javascript: 书签。请用本页「立即尝试」，或改用桌面浏览器。'
																: 'Apple blocks javascript: bookmarks. Use “Try It Now” on this page, or switch to desktop.'}
														</span>
													</li>
													<li className="flex items-start">
														<span className="text-red-400 mr-2 shrink-0">•</span>
														<span>
															...
															{isZh
																? 'PageAgent 只能操作网页内容。请先自己打开目标网站，再点书签。'
																: 'PageAgent only sees page content. Open the target site yourself first, then tap the bookmark.'}
														</span>
													</li>
												</ul>
											</div>
												...
												<div className="space-y-3 sm:space-y-4">
													...
													<div className="bg-green-500/10 border border-green-500/20 p-3 sm:p-4 rounded-lg">
														...
														<div className="flex items-center justify-center gap-3 flex-wrap">
															...
															<div dangerouslySetInnerHTML={{ __html: getInjection(cdnSource === 'china') }}></div>
														</div>
													</div>
													<div className="bg-purple-500/10 border border-purple-500/20 p-3 sm:p-4 rounded-lg">
														<p className="text-gray-300 text-sm">
															...
															{isZh ? '先打开目标网站，再点收藏夹里的 PageAgent' : 'Open the target site first, then click the PageAgent bookmark'}
														</p>
													</div>
												</div>
												<div className="bg-yellow-500/10 border border-yellow-500/20 p-3 sm:p-4 rounded-lg">
													<h4 className="font-semibold text-yellow-200 mb-3 text-sm">{isZh ? '⚠️ 注意' : '⚠️ Heads Up'}</h4>
													<ul className="space-y-2 text-sm text-gray-300">
														<li className="flex items-start text-left">
															<span className="w-1.5 h-1.5 bg-yellow-400 rounded-full mt-2 mr-2 shrink-0"></span>
															{isZh ? '使用免费测试 LLM API' : 'Uses free testing LLM API'}
														</li>
														<li className="flex items-start text-left">
															<span className="w-1.5 h-1.5 bg-yellow-400 rounded-full mt-2 mr-2 shrink-0"></span>
															{isZh ? '部分网站屏蔽脚本注入 (CSP)' : 'Some sites block script injection (CSP)'}
														</li>
														<li className="flex items-start text-left">
															<span className="w-1.5 h-1.5 bg-yellow-400 rounded-full mt-2 mr-2 shrink-0"></span>
															{isZh ? '仅识别文本' : 'Text-only understanding'}
														</li>
														<li className="flex items-start text-left">
															<span className="w-1.5 h-1.5 bg-yellow-400 rounded-full mt-2 mr-2 shrink-0"></span>
															<Link href="/docs/introduction/limitations" className="text-teal-400 hover:underline">{isZh ? '详细限制 → 文档' : 'Full limits → Docs'}</Link>
														</li>
													</ul>
												</div>
											</div>
					...
					<ul className="flex flex-wrap justify-center gap-4 sm:gap-6 text-xs sm:text-sm text-gray-400" role="list">
